...
bool test_1(float value, int bonusBalance, float discountedSum) {
	...
	if (bonus.bonusBalance == bonusBalance && bonus.discountedSum == discountedSum)
		...
		cout << endl << "Очікуваний результат: Кількість бонусів: " << bonusBalance << ", сума сплати з урахуванням знижки: " << discountedSum << endl << "Отриманий результат: Кількість бонусів: " << bonus.bonusBalance << ", сума до сплати з урахуванням знижки: " << bonus.discountedSum << endl;
	return false;
}
...
int main() {
	...
	for (int i = 0; i < 3; i++) {
		switch (i) {
			case 0:
				for (int j = 0; j < 5; j++) {
					cout << "Test " << i + 1 << "." << j + 1 << " (сум а покупки: " << value[j] << ", кількість бонусів: " << bonusBalance[j] << ", сума  сплати з урахуванням знижки: " << discountedSum[j] << ") " << (test_1(value[j], bonusBalance[j], discountedSum[j]) ? "passed" : "failed") << endl;
				}
				break;
			case 1:
				for (int j = 0; j < 5; j++) {
